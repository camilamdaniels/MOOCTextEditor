...
public class EfficientDocument extends Document {
	...
	// make sure you understand the provided method getTokens, isWord, and the provided first line of code in the processText method
	// countSyllables and isWord do not use Matcher objects or regular expressions. use the single regex provided at the start of the processText method, but don't try to use Patterns or Matchers on the individual tokens returned from the first call to getTokens
	private void processText()
	{
		...
		for (int i=0; i<tokens.size(); i++) {
			...
			if (isWord(tokens.get(tokens.size()-1)) && numSentences == 0) {
				numSentences ++;
			}
		}
